...
def GPTReport(name: str, model: str, system_instruction: str, reasoning_effort: str = "high", threaded: bool = False):
    """### 📝 GPTReport
...
def MiniTemplate(model: str, file_path: str, template_event) -> None:
    """### 📝 MiniTemplate
    Organizes text using model specified to generate a structured output.
...
- **DCB**: Data de Cessação do Benefício (se houver benefício prévio)
- **DID**: Data de Início da Doença
- **DII**: Data de Início da Incapacidade (REGRA: Se existe DCB, então DII = DCB; Se não existe DCB, então DII = DER)
...
- **HistoricoAnamnese**: Resumir em  200 palavras
...
- ✓ REGRA CRÍTICA PARA DII:
  • Se DCB existe (benefício anterior cessado) → DII = DCB
  • Se DCB não existe (primeiro requerimento) → DII = DER
- ✓ Limitar DocumentosMedicosAnalisados aos 10 mais recentes
- ✓ Respeitar limites de palavras onde especificado
- ✓ Garantir que todas as datas estejam no formato DD/MM/AAAA
...

{
    "json": {
        ...
        "TarefasExigidasUltimaAtividade": "[extrair do texto ou 'Não especificado']",
        "QuantoTempoUltimaAtividade": "[extrair do texto ou 'Não especificado']",
        "AteQuandoUltimaAtividade": "[extrair do texto no formato DD/MM/AAAA]",
        "ExperienciasLaboraisAnt": "[extrair do texto ou '-------']",
        "MotivoIncapacidade": "[máximo 3 palavras]",
        "HistoricoAnamnese": "[seção específica - máx 200 palavras]",
        "DocumentosMedicosAnalisados": "[seção específica - máx 10 itens mais recentes]",
        "ExameFisicoMental": "[seção específica - integral se existir]",
        "CausaProvavelDiagnostico": "[extrair do texto ou 'Adquirida']",
        "DID": "[extrair do texto no formato DD/MM/AAAA]",
        "DII": "[APLICAR REGRA: Se tem DCB use DCB, senão use DER - formato DD/MM/AAAA]",
        "DCB": "[data cessação benefício se houver, senão deixar vazio - formato DD/MM/AAAA]",
        "DER": "[data entrada requerimento no formato DD/MM/AAAA]",
        "DAP": "[data última atividade profissional no formato DD/MM/AAAA]",
        "CONCLUSAO PERICIAL": "[resumo da conclusão em máx 70 palavras]",
        "CIF": "[copiar escala CIF se existir ou informações relevantes]",
        "ObservacoesTratamento": "[observações sobre tratamento se existir]",
        "OutrasConsideracoes": "[outras considerações relevantes do perito]",
        "AssistenteReu": "[nome assistente da parte ré se houver]",
        "ConsideracoesAssistenteReu": "[considerações do assistente ré se houver]",
        "AssistenteAutor": "[nome assistente da parte autora se houver]",
        "ConsideracoesAssistenteAutor": "[considerações do assistente autor se houver]",
        "QuesitoDoJuizo": "[quesitos formulados pelo juízo se houver]",
        "QuesitoDoJuizoRespostas": "[respostas aos quesitos do juízo]",
        "QuesitoParteAutora": "[quesitos da parte autora se houver]"
    }
}
### EVITE ESTES PADRÕES QUE DENUNCIAM TEXTO DE IA:

  NUNCA USE:
  1. "É imperioso destacar que..."
  2. "Convém salientar que..."
  3. "Mister se faz mencionar..."
  4. "Não obstante o exposto..."
  5. "Em que pese..."
  6. "À guisa de..."
  7. Listas com mais de 3 itens numerados
  8. Subtítulos em excesso
  9. Parágrafos todos do mesmo tamanho
  10. Sempre três exemplos ou três argumentos

  PADRÕES DE IA PARA EVITAR:
  - Começar muitos parágrafos com "Além disso" ou "Ademais"
  - Usar "Por fim" no último parágrafo
  - Estrutura previsível: introdução-desenvolvimento-conclusão muito marcada
  - Excesso de cautela: "parece indicar", "possivelmente sugere", "talvez demonstre"
  - Repetir a mesma estrutura sintática em frases consecutivas

  SUBSTITUA POR:
  - Linguagem direta quando há certeza
  - Estrutura mais orgânica, menos previsível
  - Variedade real no comprimento e estrutura das frases
  - Pontuação mais diversificada (ponto e vírgula, travessões, parênteses)
  - Às vezes termine abruptamente quando o ponto foi feito
                    ...
                    contents=[
                        types.Content(
                            ...
                            parts=[
                                ...
                                types.Part(text=f"DOCUMENTO:{content}")
                            ]
                        )
                    ]
                )
